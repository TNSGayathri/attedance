...
const Register = props => {
  ...
  const [employeedetails, setEmployeedetails] = useState({ name: "", employeeid: "", password: ""})

  const handleclick = async (e) =>{
    e.preventDefault();
    // const res = await fetch('api', {/data/})
    const response = await fetch("http://localhost:4000/postemployee", {
            method: 'POST',
      headers: {
